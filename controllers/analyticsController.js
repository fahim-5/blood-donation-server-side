        ...
        summary: [
          {
            $group: {
              ...
              anonymousAmount: {
                $sum: {
                  $cond: [{ $eq: ["$isAnonymous", true] }, "$amount", 0],
                },
              },
            },
          },
...
async function getSystemAnalytics(dateFilter) {
  ...
  const performance =
    analytics[0].performance.length > 0 ? analytics[0].performance[0] : {};
  const errorCount = analytics[0].errorAnalysis.reduce(
  ...
  const healthScore = {
    performance:
      performance.avgResponseTime < 1000
        ? 100
        : Math.max(0, 100 - performance.avgResponseTime / 100),
...
async function getConversionAnalytics(dateFilter) {
  ...
  const [donorStats, requestStats, fundingStats, userJourney] =
    await Promise.all([
      ...
      User.aggregate([
        ...
        },
        {
          $group: {
            ...
            requestRate: {
              $avg: { $cond: [{ $eq: ["$hasMadeRequest", true] }, 1, 0] },
            },
            donationRate: {
              $avg: { $cond: [{ $eq: ["$hasDonated", true] }, 1, 0] },
            },
            fundingRate: {
              $avg: { $cond: [{ $eq: ["$hasFunded", true] }, 1, 0] },
            },
          },
        },
      ]),
  ...
  return {
    donorConversion: {
      ...
      activationRate:
        donorConversion.totalDonors > 0
          ? (donorConversion.activeDonors / donorConversion.totalDonors) * 100
          : 0,
      availabilityRate:
        donorConversion.activeDonors > 0
          ? (donorConversion.availableDonors / donorConversion.activeDonors) *
            100
          : 0,
    },
    requestConversion: {
      ...
      completionRate:
        requestConversion.totalRequests > 0
          ? (requestConversion.completedRequests /
              requestConversion.totalRequests) *
            100
          : 0,
    ...
    fundingConversion: {
      ...
      successRate:
        fundingConversion.totalPayments > 0
          ? (fundingConversion.successfulPayments /
              fundingConversion.totalPayments) *
            100
          : 0,
      refundRate:
        fundingConversion.successfulPayments > 0
          ? (fundingConversion.refundedPayments /
              fundingConversion.successfulPayments) *
            100
          : 0,
...
async function getPerformanceAnalytics(dateFilter) {
  ...
  return {
    ...
    resourceUsage:
      analytics[0].resourceUsage.length > 0
        ? analytics[0].resourceUsage[0]
        : {},
  };
}
...
function generateInsights(analytics) {
  ...
  if (userSummary) {
    ...
    if (activationRate < 50) {
      insights.push({
        ...
        message: `Only ${activationRate.toFixed(
          1
        )}% of users are active. Consider re-engagement campaigns.`,
        priority: "high",
      });
    }
  }
  ...
  if (donationSummary) {
    const completionRate =
      (donationSummary.completed / donationSummary.total) * 100;
    if (completionRate < 30) {
      insights.push({
        ...
        message: `Only ${completionRate.toFixed(
          1
        )}% of donation requests are completed. Consider improving donor matching.`,
        priority: "high",
      });
    }
  ...
  if (fundingSummary) {
    const anonymousPercentage =
      (fundingSummary.anonymousCount / fundingSummary.totalDonations) * 100;
    if (anonymousPercentage > 50) {
      insights.push({
        type: "info",
        title: "High Anonymous Donations",
        message: `${anonymousPercentage.toFixed(
          1
        )}% of donations are anonymous. Consider adding incentives for named donations.`,
        priority: "low",
      });
    }
  }
  ...
  if (conversionOverall) {
    if (conversionOverall.donorConversion < 20) {
      insights.push({
        ...
        message: `Only ${conversionOverall.donorConversion.toFixed(
          1
        )}% of donors are active. Consider activation campaigns.`,
        priority: "high",
      });
    }
    ...
    if (conversionOverall.requestCompletion < 40) {
      insights.push({
        ...
        message: `Only ${conversionOverall.requestCompletion.toFixed(
          1
        )}% of requests are completed. Consider improving response times.`,
        priority: "medium",
      });
    }
  }
  ...
  if (fundingSummary && fundingSummary.totalAmount > 100000) {
    insights.push({
      ...
      message: `Over ${(fundingSummary.totalAmount / 1000).toFixed(
        0
      )}K BDT raised for blood donation!`,
      priority: "low",
    });
  }
...
const exportAnalytics = asyncHandler(async (req, res, next) => {
  ...
  switch (reportType) {
    ...
    case "donation-performance":
      ...
      fileName = `donation_performance_report_${
    ...
    case "funding-analysis":
      ...
      fileName = `funding_analysis_report_${
        new Date().toISOString().split("T")[0]
      }`;
      break;
    ...
    case "system-performance":
      ...
      fileName = `system_performance_report_${
        new Date().toISOString().split("T")[0]
      }`;
      break;

    case "comprehensive":
      const analytics = await getComprehensiveAnalytics(req, res, next);
      if (analytics.success === false) {
        return next(
          new ErrorResponse("Failed to generate comprehensive analytics", 500)
        );
      }
      data = analytics.data;
      fileName = `comprehensive_analytics_report_${
        new Date().toISOString().split("T")[0]
      }`;
      break;
  ...
  await ActivityLog.logActivity({
    ...
    details: `Format: ${format}, Date range: ${startDate || "Beginning"} to ${
      endDate || "Now"
    }`,
    status: "success",
    userIp: req.ip,
