...
exports.getAllDonations = asyncHandler(async (req, res, next) => {
  // Pagination
...
exports.getDonation = asyncHandler(async (req, res, next) => {
  const donation = await DonationRequest.findById(req.params.id)
    .populate('requester', 'name email avatar phone bloodGroup district upazila')
  ...
  if (!canView) {
    return next(new ErrorResponse('Not authorized to view this donation request', 403));
  }
...
exports.createDonation = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.status === 'blocked') {
    return next(new ErrorResponse('Your account is blocked. Cannot create donation requests.', 403));
  }
...
exports.updateDonation = asyncHandler(async (req, res, next) => {
  ...
  if (!canUpdate) {
    return next(new ErrorResponse('Not authorized to update this donation request', 403));
  }

  // Don't allow updates if donation is in progress or completed
  if (donation.status !== 'pending') {
    return next(new ErrorResponse(`Cannot update donation with status: ${donation.status}`, 400));
  }
  ...
  donation = await DonationRequest.findByIdAndUpdate(
    req.params.id,
    updates,
    {
      new: true,
...
exports.deleteDonation = asyncHandler(async (req, res, next) => {
  ...
  if (!canDelete) {
    return next(new ErrorResponse('Not authorized to delete this donation request', 403));
  }

  // Don't allow deletion if donation is in progress
  if (donation.status === 'inprogress') {
    return next(new ErrorResponse('Cannot delete donation request that is in progress', 400));
  }
...
exports.acceptDonation = asyncHandler(async (req, res, next) => {
  const donation = await DonationRequest.findById(req.params.id);
  ...
  if (req.user.role !== 'donor') {
    return next(new ErrorResponse('Only donors can accept donation requests', 403));
  }

  // Check if user is blocked
  if (req.user.status === 'blocked') {
    return next(new ErrorResponse('Your account is blocked. Cannot accept donation requests.', 403));
  }

  // Check if user is available
  if (!req.user.isAvailable) {
    return next(new ErrorResponse('You are marked as unavailable for donations. Please update your availability first.', 400));
  }

  // Check if donation can be accepted
  if (donation.status !== 'pending') {
    return next(new ErrorResponse(`This donation request is already ${donation.status}`, 400));
  }
  ...
  if (req.user.bloodGroup !== donation.bloodGroup) {
    return next(new ErrorResponse(`Your blood group (${req.user.bloodGroup}) does not match the required blood group (${donation.bloodGroup})`, 400));
  }

  // Check if donor has donated recently (90 days)
  if (req.user.lastDonationDate) {
    const daysSinceLastDonation = Math.floor((new Date() - new Date(req.user.lastDonationDate)) / (1000 * 60 * 60 * 24));
    if (daysSinceLastDonation < 90) {
      return next(new ErrorResponse(`You can only donate blood every 90 days. Your last donation was ${daysSinceLastDonation} days ago.`, 400));
    }
  }
...
exports.updateStatus = asyncHandler(async (req, res, next) => {
  ...
  if (status === 'done') {
    ...
    notifications.push({
      ...
      recipientEmail: donation.requesterEmail,
      title: 'Donation Request Cancelled',
      message: `The donation request for ${donation.recipientName} has been cancelled. ${note || ''}`,
      type: 'warning',
      category: 'donation',
      priority: 'medium',
      actionUrl: `/dashboard`,
    ...
    if (donation.donor) {
      notifications.push({
        ...
        recipientEmail: donation.donorEmail,
        title: 'Donation Request Cancelled',
        message: `The donation request you accepted has been cancelled. ${note || ''}`,
        type: 'warning',
        category: 'donation',
        priority: 'medium',
        actionUrl: '/dashboard',
...
// @access  Private
exports.getMyDonationRequests = asyncHandler(async (req, res, next) => {
  const page = parseInt(req.query.page, 10) || 1;
...
exports.getDonationStats = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role !== 'admin') {
    return next(new ErrorResponse('Not authorized to view donation statistics', 403));
  }
...
// @access  Public
exports.getUrgentDonations = asyncHandler(async (req, res, next) => {
  const limit = parseInt(req.query.limit, 10) || 10;
