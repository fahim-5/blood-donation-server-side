...
const notificationController = {
  ...
  // @access  Private
  sendDonationRequestNotification: async (donationRequest, donors) => {
    try {
  ...
  // @access  Private
  sendDonationStatusNotification: async (
    donationRequest,
    donorId,
    status
  ) => {
    try {
      ...
      if (
        donorId &&
        donorId.toString() !== donationRequest.requester.toString()
      ) {
  ...
  // @access  Private
  sendUserStatusNotification: async (userId, status, adminId) => {
    try {
      const title =
  ...
  // @access  Private
  sendFundingNotification: async (funding, adminId) => {
    try {
      const notification = await notificationController.createNotification({
        recipient: adminId,
        sender: funding.user,
