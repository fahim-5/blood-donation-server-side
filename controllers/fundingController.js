...
exports.getAllFundings = asyncHandler(async (req, res, next) => {
  ...
  if (req.query.isAnonymous) filter.isAnonymous = req.query.isAnonymous === 'true';
...
// @access  Private
exports.getFunding = asyncHandler(async (req, res, next) => {
  const funding = await Funding.findById(req.params.id)
    .populate('donor', 'name email avatar')
    .populate('verifiedBy', 'name email');

  if (!funding) {
    return next(new ErrorResponse(`Funding not found with id ${req.params.id}`, 404));
  }

  // Check authorization
  if (req.user.role !== 'admin' && funding.donor._id.toString() !== req.user.id) {
...
exports.createPaymentIntent = asyncHandler(async (req, res, next) => {
  const { amount, currency = 'BDT', donationType = 'general', message = '', isAnonymous = false, displayName = '' } = req.body;
  ...
  if (amount > 100000) {
    return next(new ErrorResponse('Maximum donation amount is 100,000 BDT', 400));
  }
  ...
  if (currency === 'BDT') {
    ...
    if (stripeAmount < 50) { // 50 cents = 0.50 USD
      stripeAmount = 50;
    }
  }
...
exports.createManualFunding = asyncHandler(async (req, res, next) => {
  const {
    donorId,
    amount,
    currency = 'BDT',
  ...
  if (!donorId || !amount || !paymentMethod) {
    return next(new ErrorResponse('Donor ID, amount, and payment method are required', 400));
  }
...
// @access  Private/Admin
exports.verifyFunding = asyncHandler(async (req, res, next) => {
  const funding = await Funding.findById(req.params.id);
...
exports.processRefund = asyncHandler(async (req, res, next) => {
  ...
  if (amount > funding.amount) {
    return next(new ErrorResponse('Refund amount cannot exceed original donation amount', 400));
  }
  ...
  if (funding.paymentMethod === 'stripe' && funding.stripePaymentId) {
    try {
      ...
      return next(new ErrorResponse(`Stripe refund failed: ${stripeError.message}`, 500));
    }
  }
  ...
  await Notification.createSystemNotification({
    ...
    recipientEmail: funding.donorEmail,
    title: 'Donation Refunded',
    message: `Your donation of ${funding.formattedAmount} has been partially refunded. Refund amount: ${funding.currency === 'BDT' ? '৳' : '$'}${amount.toLocaleString()}`,
    type: 'info',
    category: 'funding',
    priority: 'medium',
    actionUrl: `/dashboard/funding/${funding._id}`,
...
// @access  Private/Admin
exports.getFundingStats = asyncHandler(async (req, res, next) => {
  // Get date ranges
        ...
        topDonors: [
          ...
          },
          {
            $project: {
              ...
              location: {
                $concat: ['$donorDetails.upazila', ', ', '$donorDetails.district'],
              },
            },
          },
        ],
...
// @access  Public
exports.getRecentDonations = asyncHandler(async (req, res, next) => {
  ...
  const recentDonations = await Funding.find({
    ...
    .select('donorName amount currency transactionDate message isAnonymous displayName')
