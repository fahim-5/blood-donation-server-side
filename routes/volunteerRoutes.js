...
router.get(
    ...
    [
        ...
        query('district').optional().trim(),
        query('upazila').optional().trim(),
        query('availability').optional().isIn(['available', 'unavailable', 'recently_donated']).withMessage('Valid availability status is required')
    ],
    validationMiddleware.validateExpressValidator,
...
router.post(
    ...
    [
        ...
        body('recipientPhone').optional().trim().isMobilePhone().withMessage('Valid phone number is required'),
        ...
        body('donationTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).withMessage('Valid donation time is required'),
        body('requestMessage').trim().notEmpty().withMessage('Request message is required'),
        body('urgencyLevel').isIn(['low', 'medium', 'high', 'critical']).withMessage('Valid urgency level is required'),
        body('requiredUnits').isInt({ min: 1, max: 10 }).withMessage('Required units must be between 1 and 10'),
...
router.get(
    ...
    validationMiddleware.validatePagination,
    [
        query('status').optional().isIn(['pending', 'inprogress', 'completed', 'cancelled']).withMessage('Valid status is required'),
...
router.put(
    ...
    validationMiddleware.validateObjectId('id'),
    [
        body('status').isIn(['pending', 'inprogress', 'completed', 'cancelled']).withMessage('Valid status is required'),
